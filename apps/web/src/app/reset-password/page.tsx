...
// Yeni şifre belirleme sayfası (Faz 4.7 dilim 3).
//
// Buraya İKİ yoldan gelinir:
//   1) `/forgot-password`'un gönderdiği e-postadaki bağlantı (kendi tetiklemesi).
//   2) Koçun `/api/coach/reset-client-password` ile tetiklediği e-postadaki bağlantı.
// İkisi de AYNI Supabase akışını kullanır: GoTrue bağlantıyı `redirectTo` (bu sayfa) adresine
// yönlendirirken tarayıcı istemcisine (`detectSessionInUrl: true`, bkz.
// `src/lib/supabase/client.ts`) kurtarma oturumu tokenlarını taşır. İstemci bunu otomatik
// işler ve `PASSWORD_RECOVERY` olayını yayınlar; biz onu (veya oluşan oturumu) bekleriz.
//
...
function describeLinkError(errorCode: string | null, description: string | null): string {
  if (errorCode === 'otp_expired') {
    return 'Bu şifre sıfırlama bağlantısının süresi dolmuş. Lütfen yeni bir bağlantı isteyin.'
  }
  if (description) return description
  return 'Bu şifre sıfırlama bağlantısı geçersiz. Lütfen yeni bir bağlantı isteyin.'
}
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (!settled) {
        settled = true
        setLinkErrorMessage(
          'Bu şifre sıfırlama bağlantısı geçersiz ya da süresi dolmuş. Lütfen yeni bir bağlantı isteyin.'
        )
        setStatus('invalid')
      }
      ...
      <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-8 shadow-2xl dark:border-zinc-800 dark:bg-[#16161d]">
        ...
        {status === 'ready' && !done && (
          <form onSubmit={onSubmit} className="space-y-5" noValidate>
            ...
            >
              {updatePassword.isPending ? 'GÜNCELLENİYOR...' : 'ŞİFREYİ GÜNCELLE'}
            </button>
          </form>
        )}
      </div>
