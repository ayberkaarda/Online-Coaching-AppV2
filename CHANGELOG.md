...
- **Faz 4.8 — Etkinlik Kaydı (tamamlandı):** koç panelinde danışan aktivitesinin (sekme görüntüleme, günlük giriş, form check yükleme, mesaj gönderme, AI üretimi, giriş/çıkış) gün hassasiyetinde görünürlüğü. İki yeni tablo (`activity_sessions`, `activity_events`, IP/user-agent/olay içeriği yok) ve tek yazma yolu `record_activity()` — `SECURITY DEFINER`, rıza kapısı fonksiyonun içinde fail-closed (`42501`), 30 dk bayatlama ve platform değişimi veritabanında yeni oturum açıyor, `aal2` RLS kapısına eklendi (14 → 16 tablo), `delete_account()` manifesti 15 → 17, pg_cron ile 180 günlük saklama/purge. Rıza kapılı `POST /api/activity` heartbeat ucu ve `POST`/`DELETE /api/activity/consent` — rıza yokken 204 No Content, kapanış sinyali `sendBeacon` değil `fetch(keepalive:true)` (Authorization taşıyabilsin diye), rıza verilmeden tek istek bile çıkmıyor. Danışan için tam ayrıntılı kendi aktivite görünümü (`/verilerim`), koç için gün bazlı özet (`CoachUserManagement`) — mahremiyet sınırı istemciden veri katmanına taşındı: `coach_activity_summary()` RPC'si `returns table(day date, total_seconds integer, event_counts jsonb)` (`returns jsonb` değil — tip güvencesi), `SECURITY INVOKER`, `service_role`'e `EXECUTE` verilmedi. Açık rıza arayüzü (`ActivityConsent.tsx`) — onay kutusu önceden işaretsiz, altı başlıklı aydınlatma metni, zorunlu ilk-giriş modalı bilinçli olarak reddedildi. B-009 (`42501` RLS reddinin sunucuya raporlanması) fırsatı değerlendirildi ve bilinçli olarak kapatılmadı.
- **Faz 4.7 — Kimlik Güvenliği (tamamlandı):** koç girişi için zorunlu TOTP çok faktörlü kimlik doğrulama + `aal2` RLS kapısı (ADR-0026) — 14 danışan-verisi tablosuna `RESTRICTIVE` politika, koç `aal2` olmadan hiçbir satırı okuyup yazamıyor, danışan yolu opt-in ve hiç etkilenmiyor. Koç tetiklemeli şifre sıfırlama (`POST /api/coach/reset-client-password`) — impersonation yok, bağlantı yalnızca danışanın kendi e-postasına gider; Bearer kimlik + sunucuda `is_coach()` doğrulaması + çift kovalı hız sınırı (koç+hedef 3/saat, koç 20/saat) + kalıcı denetim kaydı. Danışan kendi kendine şifre sıfırlama (`/forgot-password`, `/reset-password`). KVKK m.12 hesap verebilirlik denetim tablosu `coach_actions` — RLS+FORCE+sıfır politika, tek yazar `SECURITY DEFINER` `record_coach_action()`, denetim yazımı başarısız olursa sıfırlama fail-closed iptal edilir; `delete_account()` manifesti 14 → 15 tabloya çıktı.
