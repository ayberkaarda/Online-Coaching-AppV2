...
### 2.5 Server Actions (`src/app/actions.ts`) — KALDIRILDI (ölü kod)

> **Güncelleme:** Bu envanter aşağıdaki dört action'ın hiçbir yerden çağrılmadığını tespit
> ettikten sonra kullanıcı silinmesine karar verdi; `src/app/actions.ts` ve onu (tek tüketicisi
> olarak) yetim bırakan `src/lib/supabase/admin.ts` kod tabanından kaldırıldı. Aşağıdaki tablo,
> neyin neden kaldırıldığının tarihsel kaydı olarak korunuyor. Koçun yeni danışan hesabı
> oluşturması Faz 2'de koç-danışan akışıyla birlikte yeniden kurulacak.
...
supabase-js ile yapıyor. **Sonuç: dosya ve `admin.ts` kaldırıldı (bkz. yukarıdaki güncelleme notu).**
...
35 hook); `src/app/actions.ts` test edilmiyordu (dosya bu envanterin tespitinin ardından
çağrılmayan ölü kod olduğu için kaldırıldı, bkz. §2.5); `src/lib/api/proxy.ts` ve API route handler'ları
...
| 3   | `src/app/actions.ts` içindeki 4 server action **hiçbir yerden çağrılmıyor** — **KALDIRILDI**: bu tespitin ardından dosya ve tek tüketicisi olan `src/lib/supabase/admin.ts` silindi (bkz. §2.5); danışan hesabı oluşturma Faz 2'de yeniden kurulacak | grep: `@/app/actions` importu yok                                                                       | Orta — bakım yükü ve yanıltıcı güvenlik izlenimi; UI aynı işleri istemci tarafı supabase-js ile yapıyor                                                                                                                                                              |
