...
| A-07 | Medium   | Dosya yüklemede magic byte doğrulaması yok                       | Canlı: HTML gövdesi `image/jpeg`, SVG gövdesi `image/png` ile 200 (§3.7)      | Depoda aktif içerik; MIME allowlist bir gün gevşetilirse doğrudan stored XSS       | Yüklemeden önce ilk baytları doğrula (JPEG/PNG/WEBP imzası); sunucu tarafı yeniden kodlama             | **fixed** — `src/lib/upload-validation.ts` (yeni, magic-byte tespiti kabul/uzantı otoritesi)                                                                                                                                                             |
| A-08 | Medium   | İstek gövdesi sınırı yok; 10 MB'da sessiz KESME                  | Canlı: 9 MB tam parse edildi; 10 MB'da "Only the first 10MB" + 400 (§3.8)     | Bellek/CPU tüketimi (A-02 ile sınırsız tekrar); kesilen gövde yanıltıcı 400 üretir | `handleAiProxy`'de `Content-Length` kontrolü + 413; sınırı ~64 KB'a çek                                | **fixed** — `src/lib/api/proxy.ts` (`MAX_BODY_BYTES=64KB`, stream `reader.cancel()` ile erken kesme)                                                                                                                                                     |
...
| A-10 | Medium   | Güvenlik olayları loglanmıyor — tespit imkânsız                  | `src/proxy.ts` 429 dönerken hiç log yazmıyor (dosyada `logger` importu yok)   | Brute-force / limit taşması / yetki reddi görünmez; olay müdahalesi yapılamaz      | 429, tekrarlı 401 ve RLS reddi için `logger.warn` + korelasyon anahtarı (IP/kullanıcı)                 | **fixed (kısmi)** — `rate_limit_exceeded`/`auth_login_failed`/`auth_login_rate_limited` loglanıyor (`src/proxy.ts`, `src/app/api/auth/sign-in/route.ts`); RLS reddi (`42501`) için `logSecurityEvent()` hazır ama çağrı noktası yok, bkz. `AUDIT.md` §4c |
| A-11 | Medium   | Logger redact listesi PII ve sağlık verisini kapsamıyor          | `src/lib/logger.ts:20-28` — yalnızca password/token/apiKey/authorization      | Bir profil/log nesnesi loglanırsa e-posta, kilo, makro, beslenme planı düz metin   | `email`, `full_name`, `current_weight`, `macros`, `nutrition_plan`, `notes` ekle; `remove: true` koru  | **fixed** — `src/lib/logger.ts` (5→19 anahtar) + tarayıcı adaptöründe yeni `maskForConsole()`                                                                                                                                                            |
...
| A-15 | Low      | `connect-src` içinde `https://*.supabase.co` wildcard'ı          | Canlı header (§3.10)                                                          | XSS durumunda saldırganın kendi Supabase projesine sızdırma kanalı açık            | Wildcard yerine yalnızca yapılandırılan proje origin'i                                                 | **fixed** — `next.config.mjs` (yalnızca `NEXT_PUBLIC_SUPABASE_URL`'den türetilen somut origin)                                                                                                                                                           |
| A-16 | Low      | Hata mesajı iç mimariyi ifşa ediyor                              | Canlı: 503 gövdesi "Python AI sunucusuna ulaşılamadı..." (§3.11)              | Teknoloji parmak izi; hedefli saldırıyı kolaylaştırır                              | Genel mesaj + `request_id`; teknik detay yalnızca logda                                                | **fixed** — jenerik Türkçe mesaj, teknik detay yalnızca `log.error` (`tests/unit/proxy-auth.test.ts` `not.toContain('Python')`)                                                                                                                          |
...
| A-20 | Low      | Yükleme boyut/tip kontrolü istemcide de yok                      | `FormCheckTab.tsx:173-175` yalnızca `accept="image/*"`                        | Kullanıcı 5 MB üstü dosyada anlamsız hata görür; sunucu doğru reddediyor (§3.7)    | Yüklemeden önce `file.size` ve `file.type` kontrolü + Türkçe hata                                      | **fixed** — `FormCheckTab.tsx`, `src/app/profile/page.tsx` (seçim anında doğrulama + Türkçe hata)                                                                                                                                                        |
| A-21 | Low      | Dosya adından türetilen uzantı doğrudan yola giriyor             | `useFormChecks.ts:75`, `useProfile.ts:90`; canlı `poses/<uid>-x./evil` (§3.7) | Kendi ön eki altında çöp/iç içe yol; dizin dışına çıkış YOK (RLS engelliyor)       | Uzantıyı allowlist'e daralt (`jpg`, `jpeg`, `png`, `webp`, `avif`); MIME'den türet                     | **fixed** — `useFormChecks.ts`/`useProfile.ts` (uzantı `src/lib/upload-validation.ts` magic-byte sonucundan türetiliyor)                                                                                                                                 |
