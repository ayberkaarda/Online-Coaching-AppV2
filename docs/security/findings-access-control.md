...
| AC-03 | Medium   | `authenticated` rolünde `TRUNCATE` yetkisi (RLS baypas)                   | Canlı SQL G1 + `role_table_grants` dökümü (§3.3)            | RLS'e tabi olmayan toplu silme primitifi; bugün uygulama yüzeyinden erişilebilir yol yok                 | `revoke truncate, references, trigger on all tables in schema public from authenticated;`                                     | **fixed** — `supabase/migrations/20260817170000_force_rls_and_grants.sql` (bkz. `AUDIT.md` §4c, severity yeniden değerlendirmesi: canlı kanıt `truncate ... cascade` ile 11 tabloya yayılan tam veritabanı silmeyi gösterdi) |
...
| AC-06 | Low      | `FORCE ROW LEVEL SECURITY` hiçbir tabloda açık değil                      | `pg_class` dökümü + canlı FORCE testi (§3.6)                | Bugün **etkisiz**: sahip `postgres` zaten `BYPASSRLS`; gerçek sömürü senaryosu yok                       | Savunma derinliği için eklensin; asıl kazanç `postgres`'ten BYPASSRLS alınırsa doğar                                          | **fixed** — `supabase/migrations/20260817170000_force_rls_and_grants.sql` (13/13 tabloda `FORCE`; bugünkü etkisi hâlâ sıfır, bkz. `AUDIT.md` §4c)                                                                            |
...
| AC-11 | Low      | Sunucu ortam değişkeni **adları** istemci paketinde                       | `.next/static/chunks/440-322ddb754c911b00.js` (§3.9)        | Değer sızmıyor; yalnızca `SUPABASE_SERVICE_ROLE_KEY` / `AI_BACKEND_API_KEY` isimleri görünür             | `serverSchema`'yı `import 'server-only'` işaretli ayrı modüle taşı                                                            | **fixed** — `src/env.server.ts` (yeni), `src/env.ts`/`src/env.shared.ts`; kanıt: temiz build önce 4 dosyada eşleşme, sonra 0 (bkz. `AUDIT.md` §4c)                                                                           |
